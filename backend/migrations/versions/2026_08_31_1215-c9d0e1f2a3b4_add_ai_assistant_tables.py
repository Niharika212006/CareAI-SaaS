...
def upgrade() -> None:
    ...
    # 1. Create ai_conversations table if not exists (reuses existing userrole enum with create_type=False)
    if 'ai_conversations' not in tables:
        op.create_table(
            ...
            sa.Column(
                'role',
                postgresql.ENUM('PATIENT', 'DOCTOR', 'ADMIN', 'LAB_TECHNICIAN', 'PHARMACY_STAFF', name='userrole', create_type=False),
                nullable=False,
            ),
